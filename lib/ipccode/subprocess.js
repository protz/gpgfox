...
let { Ci, Cu, Cc, ChromeWorker } = require("chrome");
let { ctypes } = Cu.import("resource://gre/modules/ctypes.jsm", null);
...
//     );
    ...
    */
    ...
    function createStdinWriter() {
        ...
        stdinWorker = new ChromeWorker(self.data.url("subprocess_worker_win.js"));
    ...
    function createReader(pipe, name, callbackFunc) {
        var worker = new ChromeWorker(self.data.url("ipccode/subprocess_worker_win.js"));
...
function subprocess_unix(options) {
    ...
    function createStdinWriter() {
        debugLog("Creating new stdin worker\n");
        stdinWorker = new ChromeWorker(self.data.url("ipccode/subprocess_worker_unix.js"));
        stdinWorker.onmessage = function(event) {
            switch (event.data.msg) {
            case "info":
     ...
     */
    function createReader(pipe, name, callbackFunc) {
        var worker = new ChromeWorker(self.data.url("ipccode/subprocess_worker_unix.js"));
        worker.onmessage = function(event) {
            switch(event.data.msg) {
            case "data":
                debugLog("got "+event.data.count+" bytes from "+name+"\n");
